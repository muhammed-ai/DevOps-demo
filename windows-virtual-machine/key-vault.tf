...
resource "azurerm_key_vault_key" "deskey" {
  name         = var.module_config.vm_config.name != null ? "${var.module_config.vm_config.name}-key-${random_string.unique.result}" : (var.module_config.vm_config.name_prefix == null ? "${module.naming.key_vault_key.name_unique}" : "${var.module_config.vm_config.name_prefix != null ? "${local.name_prefix}w-key-${random_string.unique.result}" : null}")
  key_vault_id = data.azurerm_resources.kv_data.resources[0].id
  key_type     = "RSA"
  key_size     = 2048
  key_opts = [
...
# Create a random passowrd for vm admin password
resource "random_password" "adminpasswd" {
  length  = 12
  special = true
}
# Create a key vault secrect to store random admin password
resource "azurerm_key_vault_secret" "adminpasswd" {
  name         = var.module_config.vm_config.name != null ? "${var.module_config.vm_config.name}-sec-${random_string.unique.result}" : (var.module_config.vm_config.name_prefix == null ? "${module.naming.key_vault_secret.name_unique}" : "${var.module_config.vm_config.name_prefix != null ? "${local.name_prefix}w-sec-${random_string.unique.result}" : null}")
  value        = random_password.adminpasswd.result
  key_vault_id = data.azurerm_resources.kv_data.resources[0].id
}
